...
// – happy mods 
function happy(query) {
return new Promise(async(resolve,reject) => {
axios.get('https://happymod.com/search.html?q=' + query)
.then(html => {
const pp = cheerio.load(html.data);
let resultado = []
pp(".section-page-white").each(function(_,item) {
const nome = pp(item).find("a").attr("title");
const link =  'https://happymod.com' +  pp(item).find("a").attr("href");;
...
module.exports = { pinterest, happy, wallpaper, aiovideodl, wikimedia,drakor, porno, hentai, quotesAnime,  gpwhatsapp, hentaistube, nerding, apkmodhacker, xvideos, uptodown, pornhub, soundl, st, gpsrc, dafontSearch, dafontDown, igstalk,  apkmody, pornhubsrc, manga, anime, pornogratis, filme }